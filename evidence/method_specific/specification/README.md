...
Status: **seven direct constructed pilots + two external applications + first maturity audit completed / v0.2.1 current for new runs / developing retained**  
Date: 2026-09-07  
...
The key result is a two-axis separation:
...
## Protocol v0.2.1 / 프로토콜 v0.2.1

`SPEC-CH-007` supports a prospective minor revision for new runs.

New optional claim-relevant fields:
...
These fields are separate from `FINAL_SPEC_STATUS` and from the G1-G4 guardrail ledger. No v0.1/v0.2 evidence is rescored.
...
That audit predates `SPEC-APP-002` and `SPEC-CH-007`. Its verdict is preserved until a new re-audit is explicitly performed.
...
Do not simulate independent validation by another same-session retrace. The strongest next evidence is a genuinely independent review/retrace; if unavailable, add a new external corpus under v0.2.1 and preserve unfavorable or `NO_GAIN` outcomes.
